	...
	while (1)
	{
		...
		if (clientnum > 0)
		{
			for (i = 0; i<clientnum; i++)
			{
				if (stopenflag[i] == 1)
				{
					//len = VK70xNMC_GetOneChannel(i, 1, resbuffer, 1000);// To read one channel for VK701N and VK702N
					//len = VK70xNMC_GetFourChannel(i, resbuffer, 1000);// To read all channels for VK701N,  or read channe 1~4 for VK702N
					//len = VK70xNMC_GetAllChannel(i, resbuffer, 1000);// To read all channels for VK702N
					//------------------------------
					len = VK70xNMC_GetOneChannel_WithIOStatus(i, 1, resbuffer, 1000, 3);//To read one channel 2 & IO2 & IO3 for VK701N and VK702N
					//len = VK70xNMC_GetFourChannel_WithIOStatus(i, resbuffer, 1000,2);// To read all channels & IO3 for VK701N,  or read channe 1~4 & IO3  for VK702N
					//len = VK70xNMC_GetAllChannel_WithIOStatus(i, resbuffer, 1000, 1);// To read all channels & IO2 for VK702N
					//------------------------------
					if (len>0)
					{
						//printf("Read the DAQ【%d】 continous sampling number:  %d\n", i, len);
						printf("Read the DAQ【%d】 continous sampling number:%d;   ch1=%lf; IO2=%lf;IO3=%lf\n", i, len, resbuffer[0], resbuffer[1], resbuffer[2]);
						/*-----------------------
