	...
	while (1)
	{
		...
		for (i = 0; i<clientnum; i++)
		{
			if (stopenflag[i] == 0)
			{
				...
				//-------------------------------------------------
				// for VK702N
				// The "refvol" muse be set the value as 2.442 or 0
				// bitmode is 16bit or 24bit
				//-------------------------------------------------
				//st = VK70xNMC_Initialize(i, 2.442, 24, 1000, settingbuf[0], settingbuf[1], settingbuf[2], settingbuf[3]);
