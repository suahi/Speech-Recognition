...
//	int clientnum = 0;
	...
	while (1)
	{
		...
		{
			len = Server_Get_ConnectedClientNumbers(&clientnum);
			timeout_1s_uc = 0;
			Server_Get_RxTotoalBytes(&respeed, 1);
		...
		for (i = 0; i<clientnum; i++)
		{
			if (stopenflag[i] == 1)
			{
				...
				//------------------------------
				if (len>0)printf("Read the DAQ【%d】 sampling number:  %d\n", i, len);
			}
		}
		/////////////////////////////////////////////////////////////////////
		for (i = 0; i<clientnum; i++)
		{
			if (stopenflag[i] == 0)
			{
				...
				Sleep(1000);
				st = VK70xNMC_StartSampling(i);//
				printf("DAQ-%d Start to sampling function return status:【%d】\n", i, st);
			}
		}
	}
	return 0;
