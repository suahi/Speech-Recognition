			...
			if (stopenflag[i] == 1)
			{
				...
				if (len>0)
				{
					...
					if (CounterNPoints >= ReadyNPoints)
					{
						...
						Sleep(1000);// delay 1s , then start the next N points sample
						ReadyNPoints = ReadyNPoints + 100; // update the N point number
						st = VK70xNMC_StartSampling_NPoints(i, ReadyNPoints);//  start to N point sampling
						CounterNPoints = 0;
						TimesNPoints++;
