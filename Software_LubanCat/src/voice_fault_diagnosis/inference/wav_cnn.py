...
class WavCnnEngine:
    ...
    def prepare(self) -> None:
        ...
        model = AudioCNN(in_channels=5, num_classes=num_classes, use_se=bool(checkpoint.get("use_se", False)))
        ...
        if [idx_to_code.get(index) for index in range(num_classes)] != list(self.config.labels):
            raise ValueError("模型类别顺序与 lubancat_light.json 的 model.labels 不一致。")
