...
界面采用经典工业/MATLAB 桌面布局：上方读取音频和三类概率柱状图，底部左侧显示自动循环的动态示波器波形，右侧显示算法估计剩余寿命。
...
4. 解码成功后，底部左侧立即以真实时间连续循环显示 5 秒局部波形；顶部显示三类概率柱状图，右侧显示带两位小数的算法估计剩余寿命和状态等级。该播放仅为可视化，不输出声音。
