...
界面采用经典工业/MATLAB 桌面布局：上方读取音频和三类概率柱状图，底部左侧显示故障分类，右侧显示演示性剩余寿命健康指数。
...
- `bearing_health_index.joblib`：演示性健康指数随机森林回归模型。
...
4. 顶部显示三类概率柱状图；底部左侧显示类别与置信度，右侧显示健康指数和状态等级。
...
- `result.json`：三类概率、类别、置信度、整数健康指数及免责声明。

## 关于“剩余寿命”

右下方结果是**演示性剩余寿命健康指数**，是依据健康样本声学基线、分类结果与回归模型得到的稳定整数百分比：健康约为 76–93%，轴承损伤约为 31–47%，间隙异常约为 6–18%。它**不能替代真实寿命小时数**，不适用于实际检修决策或安全结论。
